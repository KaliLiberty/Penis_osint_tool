...
def dump_site(url):
    ...
    if response.status_code != 200:
        exit(Colorate.Horizontal(Colors.red_to_yellow, ("Unable to access the site.")))
    soup = BeautifulSoup(response.text, "html.parser")
    filename = url.replace('https://', '').split('.')[0] + '.html'
    print(Colorate.Horizontal(Colors.red_to_yellow, (f"The dump was saved in {filename}")))
    with open(filename, 'w', encoding='utf-8') as file:
        file.write(soup.prettify())


def generation_naxyi():
    print(Colorate.Horizontal(Colors.red_to_yellow, (f"All keys will be saved to a file. mullvad_keys.txt")))
    keys = int(input(Colorate.Horizontal(Colors.red_to_yellow, ("How many keys do you need to generate ---> "))))
...
def port_scanner():
    print(Colorate.Horizontal(Colors.red_to_yellow, ("Select mode: ")))
    print(Colorate.Horizontal(Colors.red_to_yellow, ("1 - check frequently used ports")))
    print(Colorate.Horizontal(Colors.red_to_yellow, ("2 - check the specified port")))
    mode = input(Colorate.Horizontal(Colors.red_to_yellow, ("Your choice: ")))
    if mode == '1':
        print()
        ports = [
        ...
        for port in ports:
            ...
            if result == 0:
                print(Colorate.Horizontal(Colors.red_to_yellow, (f"Port {port} open")))
            else:
                print(Colorate.Horizontal(Colors.red_to_yellow, (f"Port {port} close")))
            sock.close()
            print()
    elif mode == '2':
        port = input(Colorate.Horizontal(Colors.red_to_yellow, ("Enter the port number: ")))
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        ...
        print(Colorate.Horizontal(Colors.red_to_yellow, ("Unknown mode")))
        print()
        input(Colorate.Horizontal(Colors.red_to_yellow, ("Press Enter.....")))
...
███   ███   ███   ███    ███        ███          ███    ███ ███   ███ ███     ▄█    ███ 
 ▀█   ███   █▀    ███    ███       ▄████▀        ██████████  ▀█   █▀  █▀    ▄████████▀  
                  ███    ███                                                           
                       ...
                       Aslam_Penis!!!!! Telegram: @aslam_cik
                                   Press to Enter
"""
           ...
           ███   ███   ███   ███    ███        ███          ███    ███ ███   ███ ███     ▄█    ███ 
            ▀█   ███   █▀    ███    ███       ▄████▀        ██████████  ▀█   █▀  █▀    ▄████████▀  
                             ███    ███                                                           
                                         ...
                                         Aslam_cik  multi tool. 
                                        Oh yeah, force banner font
                Price - Free                                             Telegram: @aslam_cik
-=-=-=-=-=-=-=-= -=-=-=-=-=-=-=-= -=-=-=-=-=-=-=-= -=-=-=-=-=-=-=-= -=-=-=-=-=-=-=-= -=-=-=-=-=-=-=-= -=-=-=-=-=-=-=-=
